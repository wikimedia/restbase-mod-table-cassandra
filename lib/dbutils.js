...
dbu.makeIndexSchema = function makeIndexSchema(dataSchema, indexName) {
    ...
    if (dataSchema.conversions) {
        for (var attr in s.attributes) {
            if (dataSchema.conversions.read[attr]) {
                s.conversions.read[attr] = dataSchema.conversions.read[attr];
            }
            if (dataSchema.conversions.write[attr]) {
                s.conversions.write[attr] = dataSchema.conversions.write[attr];
            }
        }
    }
...
dbu.makeSchemaInfo = function makeSchemaInfo(schema, isMetaCF) {
    ...
    for (var att in psi.attributes) {
        ...
        var setType = /^set<(\w+)>$/.exec(type);
        var conversions = {};
        if (setType) {
            // this is a set-typed attribute
            type = setType[1];
            // generate the convertors only if the underlying type has them defined
            conversions = generateSetConvertor(dbu.conversions[type]);
        } else if (dbu.conversions[type]) {
            // this is regular type and conversion methods are defined for it
            conversions = dbu.conversions[type];
        }
        if (conversions.read) { psi.conversions.read[att] = conversions.read; }
        if (conversions.write) { psi.conversions.write[att] = conversions.write; }
    }
    ...
    // comparisons.
    psi.hash = crypto.Hash('sha1')
        .update(stableStringify(psi))
...
dbu.convertRows = function convertRows(rows, schema) {
    var readConversions = schema.conversions.read;
    for (var i = 0; i < rows.length; i++) {
        ...
        for (var ri = 0; ri < rowKeys.length; ri++) {
            ...
            if (!/^_/.test(att)) {
                if (readConversions[att] && row[att] !== null) {
                    newRow[att] = readConversions[att](row[att]);
                } else {
                    newRow[att] = row[att];
                }
            } else if (att === '_ttl') {
...
dbu.makeRawRequest = function(internalReq, extendFields) {
    ...
    var writeConversions = conversions.write;
    Object.keys(attrs).forEach(function(key) {
        var conv = writeConversions[key];
        if (conv) {
            attrs[key] = conv(attrs[key]);
        }
    });
    return clonedReq;
};
...
dbu.buildCondition = function buildCondition(predicates, schema, noConvert) {
    var convert;
    if (noConvert || Object.keys(schema.conversions.write).length === 0) {
        convert = function(key, val) {
            return val;
        };
    } else {
        convert = function(key, val) {
            var convObj = schema.conversions.write[key];
            if (convObj) {
                return convObj(val);
            } else {
                return val;
    ...
    // Order predicates
    var predKeys = schema.iKeys.concat(Object.keys(predicates)
            .filter(function(key) {
                return schema.iKeyMap[key] === undefined
                    && schema.staticKeyMap[key] === undefined;
            })
            .sort());

    var params = [];
    var conjunctions = [];
    for (var i = 0; i < predKeys.length; i++) {
        var predKey = predKeys[i];
...
dbu.buildPutQuery = function(req, noConvert) {
    ...
    var writeConversions = schema.conversions.write || {};
    Object.keys(attributes).forEach(function(key) {
        ...
        if (val !== undefined && schema.attributes[key]) {
            if (!schema.iKeyMap[key]) {
                ...
                var conversion = writeConversions[key];
                if (!noConvert && conversion) {
                    val = conversion(val);
                }
...
// Cache for buildGetQuery
function GetCache() {
    this._cache = {};
}

GetCache.prototype._convertAttributes = function(obj, schema, inAttribute) {
    var key = '';
    // Ignore values
    var iKeys = inAttribute ? Object.keys(obj).sort() : schema.iKeys;
    var params = [];
    key += '{';
    for (var i = 0; i < iKeys.length; i++) {
        var attribute = iKeys[i];
        var attrVal = obj[attribute];
        if (attrVal === undefined) {
            continue;
        }
        key += '|' + attribute + ':';
        if (attrVal && attrVal.constructor === Object
                && !inAttribute) {
            var res = this._convertAttributes(attrVal, schema, true);
            key += res.key;
            params = params.concat(res.params);
        } else {
            var conversion = schema.conversions.write[attribute];
            if (conversion) {
                params.push(conversion(attrVal));
            } else {
                params.push(attrVal);
            }
        }
...
GetCache.prototype.get = function(req, options, schema) {
    var key = schema.hash + req.keyspace;
    if (options && Object.keys(options).length) {
        key += '|' + stableStringify(options);
    }
    var objKeys = Object.keys(req.query).sort();
    var params;
    for (var i = 0; i < objKeys.length; i++) {
        var objKey = objKeys[i];
        var val = req.query[objKey];
        key += '|' + objKey + ':';
        if (objKey === 'attributes') {
            var attrRes = this._convertAttributes(val, schema);
            key += attrRes.key;
            params = attrRes.params;
        } else {
            if (typeof val === 'object') {
                key += stableStringify(val);
            } else {
                key += val;
            }
        }
...
GetCache.prototype.set = function(key, val) {
    this._cache[key] = val;
};

var getCache = new GetCache();
...
dbu.buildGetQuery = function(req, options) {
    ...
    var attributes = query.attributes = query.attributes || {};
    if (req.columnfamily !== 'meta') {
        attributes._domain = req.domain;
    }
    ...
    var cacheRes = getCache.get(req, options, schema);
    if (cacheRes.val) {
        // Can use the cached result. Return.
        return {
            cql: cacheRes.val,
            params: cacheRes.params
        };
    }

    var projAttrs = Object.keys(schema.attributes);
    var projCQL = projAttrs.map(dbu.cassID).join(',');

    if (query.proj) {
        if (Array.isArray(query.proj)) {
            projAttrs = query.proj;
            projCQL = projAttrs.map(dbu.cassID).join(',');
    ...
    if (pattern === 'timeseries' && db.cassandraVersion && semver.valid(db.cassandraVersion)) {
        ...
        if (!semver.satisfies(db.cassandraVersion, '>= 2.0.11 < 2.1.0 || >= 2.1.1')) {
            ...
            db.log('warn/updateHints',
                    'Using STCS instead of DTCS for "timeseries" update pattern: '
                    + 'DTCS not supported by cassandra v' + db.cassandraVersion);
        }
    }
