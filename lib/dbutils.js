...
TimeUuid.prototype.toJSON = TimeUuid.prototype.toString;
Uuid.prototype.toJSON = Uuid.prototype.toString;
Integer.prototype.toJSON = Integer.prototype.toNumber; // FIXME: use string too
BigDecimal.prototype.toJSON = BigDecimal.prototype.toString;
...
dbu.tidNode = new Buffer([0x01, 0x23, 0x45, 0x67, 0x89, 0xab]);
dbu.tidClock = new Buffer([0x12, 0x34]);

dbu.tidFromDate = function tidFromDate(date, useCassTicks) {
    return new TimeUuid(date, useCassTicks ? null : 0, dbu.tidNode, dbu.tidClock);
};

dbu.tidNanoTime = function(tid) {
    var datePrecision = tid.getDatePrecision();
    return datePrecision.date.getTime() * 10000 + datePrecision.ticks;
};
