...
dbu.makeSchemaInfo = function makeSchemaInfo(schema, isMetaCF) {
    ...
    psi.iKeys = dbu.indexKeys(psi.index).sort();
    psi.iKeyMap = {};
    psi.staticKeyMap = {};
...
dbu.buildCondition = function buildCondition(predicates, schema, noConvert) {
    ...
    var params = [];
    var conjunctions = [];
    var predKeys = Object.keys(predicates).sort();
...
GetCache.prototype._convertAttributes = function(obj, schema, inAttribute) {
    var key = '';
    // Ignore values
    var attributes = Object.keys(obj).sort();
    var params = [];
    key += '{';
...
GetCache.prototype.get = function(req, options) {
    var key = req.schema.hash + req.keyspace;
    ...
    for (var i = 0; i < objKeys.length; i++) {
        ...
        if (objKey === 'attributes') {
            var attrRes = this._convertAttributes(val, req.schema);
            key += attrRes.key;
...
dbu.buildGetQuery = function(req, options) {
    ...
    var cacheRes = getCache.get(req, options);
    if (cacheRes.val) {
