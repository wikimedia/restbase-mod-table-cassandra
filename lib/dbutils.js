...
dbu.makeIndexSchema = function makeIndexSchema(dataSchema, indexName) {
    ...
    const index = dataSchema.secondaryIndexes[indexName];
    const s = {
        name: indexName,
        attributes: {},
        index,
    ...
    if (dataSchema.conversions) {
        for (const attr in s.attributes) {
...
dbu.makeSchemaInfo = function makeSchemaInfo(schema, isMetaCF) {
    ...
    if (!isMetaCF) {
        ...
        if (psi.secondaryIndexes) {
            Object.keys(psi.secondaryIndexes).forEach((idxName) => {
                const idx = psi.secondaryIndexes[idxName];
    ...
    Object.keys(psi.secondaryIndexes).forEach((si) => {
        psi.secondaryIndexes[si] = dbu.makeIndexSchema(psi, si);
        const idx = psi.secondaryIndexes[si];
        idx.iKeys.forEach((att) => {
...
dbu.secondaryIndexTableName = (idx) => `idx_${idx}_ever`;
