...
DB.prototype.buildGetQuery = function(keyspace, req, consistency, table, schema) {
    ...
    if (req.order) {
        ...
        schema.index.forEach(function(elem) {
            if (elem.type === 'range') {
                ...
                orderTerms.push(cassID(elem.attribute) + ' ' + dir);
            }
        });
...
DB.prototype.indexReads = function(keyspace, req, consistency, table, startKey, finalRows) {
    ...
    var buildResult = this.buildGetQuery(keyspace, newIndexReq, consistency,
            table, cachedSchema);
...
DB.prototype._getSecondaryIndex = function(keyspace, req, consistency, table, buildResult){
    ...
    .then(function(results) {
        ...
        for ( var rowno in results.rows ) {
            ...
            queries.push(self.buildGetQuery(keyspace, newReq, consistency, table, cachedSchema));
            newReq.attributes = {};
        }
...
DB.prototype._get = function (keyspace, req, consistency, table, schema) {
    ...
    var buildResult = this.buildGetQuery(keyspace, req, consistency, table, schema);
...
DB.prototype._put = function(keyspace, req, consistency, table ) {
    ...
    .then(function(result) {
        ...
        if (schema.secondaryIndexes) {
            self._rebuildIndexes(keyspace, req, schema);
        }
 ...
 */
DB.prototype._rebuildIndexes = function (keyspace, req, schema, indexes, limit) {
    var self = this;
    if (!indexes) {
    ...
    if (indexes.length) {
        ...
        var mainRebuild = new Promise(function(resolve, reject) {
            ...
            var dataQuery = self.buildGetQuery(keyspace, dataReq, consistency, 'data', schema);
