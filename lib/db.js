...
DB.prototype._makeInternalRequest = function(domain, table, query, consistency) {
    ...
    var keyspace = this.keyspaceName(domain, table);
    var opts = {
        domain: domain,
        table: table,
        keyspace: keyspace,
 ...
 */
DB.prototype.keyspaceName = function(domain, table) {
...
DB.prototype.get = function(domain, query) {
    ...
    .then(function(req) {
        var options = query.withTTL ? { withTTL: true } : undefined;
        return self._getRaw(req, options)
        .then(function(res) {
...
DB.prototype._put = function(req) {
    ...
    if (schema.revisionRetentionPolicy
        ...
        update = update.then(function() {
            return self._deleteOlderContent(req);
        });
    }
...
DB.prototype._deleteOlderContent = function(req) {
    var self = this;
    function deleteRow(row) {
        var cql = 'DELETE FROM ' + dbu.cassID(req.keyspace) + '.'
                  + dbu.cassID(req.columnfamily) + ' WHERE ';
        var predicates = {};
        req.schema.iKeys.forEach(function(att) {
            predicates[att] = row[att];
        });
        var condition = dbu.buildCondition(predicates, req.schema, true);
        return self.client.execute_p(cql + condition.cql, condition.params, { prepare: true })
        .catch(function(e) {
            self.log('error/table/cassandra/revisionRetentionPolicyUpdate', e);
        });
    }
    ...
    for (var idx = 0; idx < req.schema.iKeys.length; idx++) {
        var att = req.schema.iKeys[idx];
    ...
    var dataGetInfo = dbu.buildGetQuery(dataGetReq, { limit: 3 });
    return dbu.eachRow(
        self.client,
        dataGetInfo.cql,
        dataGetInfo.params,
...
DB.prototype._backgroundUpdates = function(req, limit, indexes) {
    ...
    return self._getRaw(newerRebuildRequest, { withTTL: true })
    ...
    .then(function() {       // Query for 'limit' subsequent records
        ...
        var dataGetInfo = dbu.buildGetQuery(dataGetReq, { withTTL: true, limit: limit });

        return dbu.eachRow(
            self.client,
            dataGetInfo.cql,
            dataGetInfo.params,
            { retries: 3, fetchSize: 5, log: self.log, withTTL: true },
...
DB.prototype._migrateIfNecessary = function(req, currentSchemaInfo, newSchema, newSchemaInfo) {
    ...
    if (currentSchemaInfo.hash === newSchemaInfo.hash) {
        ...
        return P.try(function() {
            return migrator.migrate(req, currentSchemaInfo, newSchemaInfo);
        })
        ...
        .catch(function(error) {
            var newErr = new dbu.HTTPError({
                status: 400,
                body: {
                    type: 'bad_request',
                    title: 'The table already exists, and it cannot be ' +
                           'upgraded to the requested schema (' + error + ').',
                    keyspace: req.keyspace,
                    schema: newSchema,
                    stack: error.stack,
                }
            });
            self.log('error/cassandra/table_update', newErr);
            throw newErr;
        });
    }
};
...
DB.prototype._setReplication = function(domain, table, options) {
    var keyspace = this.keyspaceName(domain, table);
    var cql = "ALTER KEYSPACE " + dbu.cassID(keyspace) +
              " WITH replication = " + this._createReplicationOptionsCQL(options);
 ...
 * @return {object} promise that resolves when complete
 */
DB.prototype.updateReplicationIfNecessary = function(domain, table, options) {
