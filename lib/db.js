...
DB.prototype._get = function (keyspace, req, consistency, table, schema) {
    ...
    if (req.paging) {
        var rows = [];
        var options = {consistency: consistency, fetchSize: req.paging};
        if (req.next) {
            options.pageState = req.next;
        }
                ...
                self.client.eachRow(buildResult.query, buildResult.params, options,
                    function(n, result){
                        schema.conversions.read.forEach(function(conv) {
                            var val = result[conv.att];
                            if (val) {
                                result[conv.att] = conv.fn(val);
                            }
                            if (!result._del) {
                                rows.push(result);
                            }
                        ...
                        if (err) {
                            ...
                            //XXX: convert result.meta.pageState to a url or token 
                            resolve({
                                count: rows.length,
                                items: rows,
                                next: result.meta.pageState
                            });
                       }
                    }
                );
