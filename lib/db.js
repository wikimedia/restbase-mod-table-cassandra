...
class DB {
    ...
    _put(req) {
        ...
        let update = this.client.execute(queryInfo.cql, queryInfo.params, queryOptions);
     ...
     *
     * Queries for sibling revisions (1 newer and up to 'limit' older), and apply
     * to the table's revision retention policy (RevisionPolicyManager).
     *
    ...
    _backgroundUpdates(req, limit) {
        ...
        const handler = new UpdateHandler([policyManager]);
 ...
 */
DB.prototype._shouldRunBackgroundUpdates = (schema) => {
    // If the retention policy is 'all' (i.e. there are no revisions to cull),
    // then there is no need to run background updates.
    return schema.revisionRetentionPolicy
